...
struct inodo {
    ...
    char padding[T_INODO - 3 * sizeof (time_t) - 19 * sizeof (unsigned int)]; //T_INODO-88
};
...
struct STAT {
    ...
    unsigned int tamEnBytesLog; //Tamaño en bytes lógicos
    unsigned int numBloquesOcupados; //Cantidad de bloques ocupados en la zona de datos
    char padding[T_INODO - 3 * sizeof (time_t) - 4 * sizeof (unsigned int)];
};
...
int traducir_bloque_inodo(unsigned int inode, unsigned int blogico, unsigned int *bfisico, char reservar);
int liberar_inodo(unsigned int inode);
int liberar_bloques_inodo(unsigned int inode, unsigned int blogico);
