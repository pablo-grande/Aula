; ---------------------------------------------------------
; La idea es crear el estado cuando se llama a LANZAR
; y repintar la pantalla en cada operación, así evitamos
; guarradas del estilo borrar recta.
;
; Los encabalgamientos es porque la pantalla de LISP
; no permite más allá de esa longitud y no acaba la sentencia.
;
; ---------------------------------------------------------
...
(defun crearPropietats ()
	...
	(putprop 'escenari (curva 9) 'corba)			; Corba del drac nivell 7
	...
	(putprop 'escenari 1 'torn)						; 1 = Torn del jugador // 0 = Torn de la màquina
	...
	(putprop 'cano1 0 'encertat) 	; 0 No encertat // 1 Encertat
	(putprop 'cano1 'cano1 'nom)	; Nom del canó 1
	(putprop 'cano1 40 'darrerTir)	; Força darrer tir cano 1
	(putprop 'cano1 40 'darrerInc)	; Força darrer increment cano 1
	...
	(putprop 'cano2 0 'encertat) 	; 0 No encertat // 1 Encertat
	(putprop 'cano2 'cano2 'nom)	; Nom del canó 2
	(putprop 'cano2 40 'darrerTir)	; Força darrer tir cano 2
	(putprop 'cano2 40 'darrerInc)	; Força darrer increment cano 2
)
...
; DIU SI ÉS EL TORN DEL JUGADOR
(defun meuTorn ()
	(cond	(	(eq (get 'escenari 'torn) 1) t
			)(	(eq (get 'escenari 'torn) 0) nil
			)
	)
)

; AJUSTA EL DISPAR DEL CANÓ
(defun ajustaDispar (c)
	(cond	(	(eq (get c 'nom) 'cano1)
				(cond	(	(<= (get c 'px) (get 'cano2 'xcc))
							(putprop c (+ (get c 'darrerTir) 10) 'darrerTir)
						)(	(not (eq (get c 'py) (get 'cano2 'ycp)))
							(puja c 10)
						)(	(< (get c 'px) (get 'cano2 'xcp))
							(putprop c (+ (get c 'darrerTir) (get c 'darrerInc)) 'darrerTir)
							(putprop c (/ (get c 'darrerInc) 2) 'darrerInc)
						)(	(> (get c 'px) (get 'cano2 'xcp))
							(putprop c (- (get c 'darrerTir) (get c 'darrerInc)) 'darrerTir)
							(putprop c (/ (get c 'darrerInc) 2) 'darrerInc)
						)
...
(defun simula (c v)
	...
	(color 0 0 0 135 206 250)
	(cond	(	(meuTorn)
				(cond	(	(eq (get c 'encertat) 0)
							(putprop 'escenari 0 'torn)
							(cond	(	(eq (get c 'nom) 'cano1)
										(simula 'cano2 (get 'cano2 'darrerTir))
										(ajustaDispar 'cano2)
										(print 'asdf)
									)(	(eq (get c 'nom) 'cano2)
										(simula 'cano1 (get 'cano1 'darrerTir))
										(ajustaDispar 'cano1)
									)
									
							)
							(putprop 'escenari 1 'torn)
						)
				)
			)
	)
